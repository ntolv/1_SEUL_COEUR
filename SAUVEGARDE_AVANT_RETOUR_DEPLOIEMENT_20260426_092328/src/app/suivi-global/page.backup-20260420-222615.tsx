...
function aggregateRows(rows: EncaissementRow[]) {
  ...
  for (const row of rows) {
    const rubriqueKey = normalize(row.rubrique_nom) || String(row.rubrique_id ?? "");
    const key = `${row.personne_id}__${row.type_personne}__${rubriqueKey}`;

    const current: EncaissementRow = {
      ...row,
    ...
    const existingTime = getMonthTime(existing.mois_reference);
    const currentTime = getMonthTime(current.mois_reference);
    const latest = currentTime >= existingTime ? current : existing;

    const montant_attendu =
      Number(existing.montant_attendu || 0) + Number(current.montant_attendu || 0);
    const montant_encaisse =
      Number(existing.montant_encaisse || 0) + Number(current.montant_encaisse || 0);
    const reste = montant_encaisse - montant_attendu < 0
      ? Math.abs(montant_encaisse - montant_attendu)
      : 0;
    ...
    map.set(key, {
      ...
      montant_encaisse,
      reste,
      statut: montant_encaisse - montant_attendu >= 0 ? "A jour" : "En retard",
    });
  }
...
export default function SuiviGlobalPage() {
  ...
  const stats = useMemo(() => {
    const totalAttendu = aggregatedRows.reduce((sum, row) => sum + Number(row.montant_attendu || 0), 0);
    const totalEncaisse = aggregatedRows.reduce((sum, row) => sum + Number(row.montant_encaisse || 0), 0);
    const totalReste = aggregatedRows.reduce((sum, row) => sum + Number(row.reste || 0), 0);
    const nombreAjour = aggregatedRows.filter((row) => row.statut === "A jour").length;
    const nombreEnRetard = aggregatedRows.filter((row) => row.statut === "En retard").length;
    ...
    return {
      ...
      nombreAjour,
      nombreEnRetard,
      nombreTotal: aggregatedRows.length,
    };
  }, [aggregatedRows]);
                    ...
                    {groupedRows.map((groupe) => (
                      <React.Fragment key={groupe.personneId}>
                        {groupe.rows.map((row, index) => (
                          <tr
                            key={`${row.personne_id}-${row.rubrique_id ?? row.rubrique_nom}-${index}`}
