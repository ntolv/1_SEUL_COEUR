...
function aggregateRows(rows: EncaissementRow[]) {
  const map = new Map<string, EncaissementRow>();

  for (const rawRow of rows) {
    const row: EncaissementRow = {
      ...rawRow,
      nom_complet: rawRow.nom_complet ?? "",
      telephone: rawRow.telephone ?? null,
      email: rawRow.email ?? null,
      mois_reference: rawRow.mois_reference ?? "",
      rubrique_nom: rawRow.rubrique_nom ?? "",
      rubrique_code: rawRow.rubrique_code ?? null,
      montant_attendu: Number(rawRow.montant_attendu || 0),
      montant_encaisse: Number(rawRow.montant_encaisse || 0),
      reste: Number(rawRow.reste || 0),
    };

    const rubriqueKey =
      normalize(row.rubrique_nom) ||
      normalize(row.rubrique_code) ||
      String(row.rubrique_id ?? "");

    const key = `${row.personne_id}__${row.type_personne}__${rubriqueKey}`;
    ...
    const existingMonthTime = getMonthTime(existing.mois_reference);
    const currentMonthTime = getMonthTime(row.mois_reference);
    const latestRow = currentMonthTime >= existingMonthTime ? row : existing;

    const montant_attendu =
      Number(existing.montant_attendu || 0) + Number(row.montant_attendu || 0);
    const montant_encaisse =
      Number(existing.montant_encaisse || 0) + Number(row.montant_encaisse || 0);

    const diff = montant_encaisse - montant_attendu;
    const reste = diff < 0 ? Math.abs(diff) : 0;
    ...
    map.set(key, {
      ...
      montant_encaisse,
      reste,
      statut: diff >= 0 ? "A jour" : "En retard",
    });
  }
...
export default function SuiviGlobalPage() {
  ...
  const mergedRows = useMemo(() => {
    return aggregateRows(filteredRows);
  }, [filteredRows]);
  ...
  const groupedRows = useMemo(() => {
    ...
    mergedRows.forEach((row) => {
      const personKey = `${row.personne_id}__${row.type_personne}`;

      if (!groups.has(personKey)) {
        groups.set(personKey, []);
      }
      groups.get(personKey)!.push(row);
    });

    return Array.from(groups.entries()).map(([personKey, personRows]) => ({
      personKey,
  ...
  const stats = useMemo(() => {
    const totalAttendu = mergedRows.reduce(
      (sum, row) => sum + Number(row.montant_attendu || 0),
      0
    );
    const totalEncaisse = mergedRows.reduce(
      (sum, row) => sum + Number(row.montant_encaisse || 0),
      0
    );
    const totalReste = mergedRows.reduce(
      (sum, row) => sum + Number(row.reste || 0),
      0
    );
    const nombreAjour = mergedRows.filter((row) => row.statut === "A jour").length;
    const nombreEnRetard = mergedRows.filter((row) => row.statut === "En retard").length;
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {groupedRows.map((groupe) => (
                      <React.Fragment key={groupe.personKey}>
                        {groupe.rows.map((row, index) => (
                          <tr
                            key={`${groupe.personKey}-${row.rubrique_id ?? normalize(row.rubrique_nom)}-${index}`}
