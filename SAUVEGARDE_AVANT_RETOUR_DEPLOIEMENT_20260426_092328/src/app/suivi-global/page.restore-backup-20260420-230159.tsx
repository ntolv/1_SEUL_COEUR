...
function fusionnerDoublonsParRubrique(rows: EncaissementRow[]) {
  ...
  for (const row of rows) {
    ...
    const existingMonth = getMonthTime(existing.mois_reference);
    const currentMonth = getMonthTime(row.mois_reference);
    const mois_reference =
      currentMonth >= existingMonth ? row.mois_reference : existing.mois_reference;
...
export default function SuiviGlobalPage() {
  ...
  const mergedRows = useMemo(() => {
    return fusionnerDoublonsParRubrique(filteredRows);
  }, [filteredRows]);
  ...
  const groupedRows = useMemo(() => {
    ...
    mergedRows.forEach((row) => {
      const groupKey = `${row.personne_id}__${row.type_personne}`;
    ...
    return Array.from(groups.entries()).map(([groupKey, personRows]) => ({
      groupKey,
                    ...
                    {groupedRows.map((groupe) => (
                      <React.Fragment key={groupe.groupKey}>
                        {groupe.rows.map((row, index) => (
                          <tr
                            key={`${groupe.groupKey}-${row.rubrique_id ?? row.rubrique_nom}-${index}`}
