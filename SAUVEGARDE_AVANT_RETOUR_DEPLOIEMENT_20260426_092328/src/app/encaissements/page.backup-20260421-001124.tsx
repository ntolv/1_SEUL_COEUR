...
const DEFAULT_RUBRIQUES: RubriqueRow[] = [
  { code: "TONTINE_GRAND_CAHIER", nom: "Tontine Grand Cahier" },
  { code: "TONTINE_PETIT_CAHIER", nom: "Tontine Petit Cahier" },
  { code: "REPAS", nom: "Repas" },
  { code: "ANNIVERSAIRE", nom: "Anniversaire" },
  { code: "FONDS_DE_ROULEMENT", nom: "Fonds de roulement" },
  { code: "PROJET", nom: "Projet" },
  { code: "SECOURS", nom: "Secours" },
  { code: "FRAIS_INSCRIPTION", nom: "Frais inscription" },
];
...
function getPersonKey(personne: Pick<PersonneRow, "id" | "type_personne">) {
  return `${personne.type_personne}::${personne.id}`;
}

function dedupePersonnes(list: PersonneRow[]) {
  const byKey = new Map<string, PersonneRow>();
...
function mergeRubriques(rows: RubriqueRow[]) {
  const byKey = new Map<string, RubriqueRow>();

  for (const item of rows) {
    const nom = (item.nom ?? "").trim();
    if (!nom) continue;

    const key = normalizeName(item.code || nom);
    if (!byKey.has(key)) {
      byKey.set(key, {
        id: item.id ?? null,
        code: item.code ?? null,
...
export default function EncaissementsPage() {
  ...
  async function chargerPersonnes() {
    const { data: uniques, error: errUniques } = await supabase.rpc(
      "fn_encaissement_personnes_uniques"
    );

    if (!errUniques && Array.isArray(uniques) && uniques.length > 0) {
      const list = dedupePersonnes(
        uniques.map((row: any) => ({
          id: String(row.personne_id ?? row.id ?? ""),
          nom_complet: row.nom_complet ?? "",
          telephone: row.telephone ?? null,
          type_personne:
            row.type_personne === "PREINSCRIT" ? "PREINSCRIT" : "MEMBRE",
        }))
      );
      setPersonnes(list);
      return;
    ...
    setPersonnes(dedupePersonnes(list));
  }
  ...
  async function chargerRubriques() {
    ...
    if (error || !data || data.length === 0) {
      setRubriques(mergeRubriques(DEFAULT_RUBRIQUES));
      return;
    }

    const backendRubriques = (data as any[]).map((row) => ({
      id: row.id ?? null,
      code: row.code ?? null,
      nom: row.nom ?? "",
    }));

    setRubriques(mergeRubriques([...DEFAULT_RUBRIQUES, ...backendRubriques]));
  }
    ...
    try {
      ...
      const rpcPayload = {
        ...
        p_ventilations: ventilations.map((v) => ({
          ...
          rubrique_nom: v.rubrique_nom,
          rubrique_code: v.rubrique_code,
  ...
  async function encaisserAttendu() {
    ...
    const rubrique = rubriques.find(
      (r) => normalizeName(r.nom) === normalizeName(expectedRubriqueNom)
    );
    ...
    try {
      const { data, error } = await supabase.rpc("fn_encaisser", {
        ...
        p_ventilations: [
          {
            rubrique: rubrique?.nom ?? expectedRubriqueNom,
            rubrique_nom: rubrique?.nom ?? expectedRubriqueNom,
            rubrique_code: rubrique?.code ?? null,
      ...
      const newSelectedKey = getPersonKey(expectedPersonne);
            ...
            <div className="grid gap-4 xl:grid-cols-3">
              ...
              <div>
                ...
                >
                  <option value="">Sélectionner</option>
                  {rubriques.map((r) => (
                    <option key={r.code ?? r.nom} value={r.nom}>
                      {r.nom}
                    </option>
                  ))}
                </select>
