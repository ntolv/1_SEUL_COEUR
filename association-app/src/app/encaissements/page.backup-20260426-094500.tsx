...
const QUICK_BUTTONS: Record<string, number[]> = {
  ...
  Anniversaire: [30],
  Epargne: [50, 100],
  "Frais inscription": [20],
  Projet: [10, 50],
  Secours: [10, 50],
  "Tontine Grand Cahier": [50, 100, 500],
  "Tontine Petit Cahier": [50, 100],
...
export default function EncaissementsPage() {
  ...
  async function chargerRubriques() {
    ...
    const merged = mergeRubriques(backendRubriques);

    if (merged.length === 0) {
      throw new Error("Aucune rubrique active trouvée dans encaissement_rubriques.");
    }
